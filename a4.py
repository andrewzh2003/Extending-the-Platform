                ...
                ## username
                    ...
                    if first == list1[3]:
                        ...
                        if "@weather" in words:
                            ...
                            open_weather.set_apikey('3e6e18ffbc23e0c8919d6484aafc5da8')
                            open_weather.load_data()
                            result = open_weather.transclude(words)
                            po = Post(result)
                            x.add_post(po) 
                            x.save_profile(full_name)
                            print("post created!")
                        elif "@lastfm" in words:
                            lastfm = LastFM()
                            lastfm.set_apikey('07808c7623679f2fce1fbbb12aab3cbb')
                            lastfm.load_data()
                            result = lastfm.transclude(words)
                            po = Post(result)
                            x.add_post(po) 
                            x.save_profile(full_name)
                            print("post created!")
